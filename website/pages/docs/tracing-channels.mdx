...
- Tracing payloads keep GraphQL.js objects such as `schema`, `document`, and
  `result` non-enumerable so in-process subscribers can still read them, while
  `structuredClone` (used by Cloudflare Tail Workers) only copies clone-safe
  enumerable fields such as operation names and field paths.
